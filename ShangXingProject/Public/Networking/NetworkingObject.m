...
static ResponseType a_responseType = kResponseTypeJSON;
static RequestType  a_requestType  = kRequestTypeJSON;
static NetworkStatus a_networkStatus = kNetworkStatusUnknown;
...
+ (URLSessionTask *)_requestWithUrl:(NSString *)url
    ...
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil) {
            NetworkingLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
        }
    } else {
    ...
    if (httpMethod == 1) {
        if (a_cacheGet) {
            if (a_shoulObtainLocalWhenUnconnected) {
                if (a_networkStatus == kNetworkStatusNotReachable ||  a_networkStatus == kNetworkStatusUnknown ) {
                    id response = [NetworkingObject cahceResponseWithURL:absolute
                                                          parameters:params];
                    if (response) {
                        if (success) {
...
+ (URLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        NetworkingLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        NetworkingLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
...
+ (URLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (error == nil) {
            ...
            if ([self isDebug]) {
                NetworkingLog(@"Download success for url %@",
            ...
            if ([self isDebug]) {
                NetworkingLog(@"Download fail for url %@, reason : %@",
                                [self absoluteUrlWithPath:url],
                                [error description]);
            }
        }
...
+ (void)detectNetwork{
    ...
    [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        if (status == AFNetworkReachabilityStatusNotReachable){
            a_networkStatus = kNetworkStatusNotReachable;
        }else if (status == AFNetworkReachabilityStatusUnknown){
            a_networkStatus = kNetworkStatusUnknown;
        }else if (status == AFNetworkReachabilityStatusReachableViaWWAN){
            a_networkStatus = kNetworkStatusReachableViaWWAN;
        }else if (status == AFNetworkReachabilityStatusReachableViaWiFi){
            a_networkStatus = kNetworkStatusReachableViaWiFi;
        }
    }];
}
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        if (data) {
            cacheData = data;
            NetworkingLog(@"Read data from cache for url: %@\n", url);
        }
    }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
    if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
            ...
            [[NSFileManager defaultManager] createDirectoryAtPath:directoryPath
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error];
            if (error) {
                NetworkingLog(@"create cache dir error: %@\n", error);
                return;
            }
        }
        ...
        if (data && error == nil) {
            ...
            if (isOk) {
                NetworkingLog(@"cache file ok for request: %@\n", absoluteURL);
            } else {
                NetworkingLog(@"cache file error for request: %@\n", absoluteURL);
            }
        }
    }
}
